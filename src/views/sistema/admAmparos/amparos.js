...
import { getAll as getPlanesAll } from "../../../redux/planes/actions";
...
export class amparosScreen extends connect(store, EVENTO_EJECUTAR_TAREA_DAR_CUMPLIMIENTO, TAREA_DAR_CUMPLIMIENTO_OK, TAREA_DAR_CUMPLIMIENTO_ERROR, FILTRO_AMPAROS, PLAN_ADD, SACAR_FILTRO_AMPAROS, EVENTO_MOSTRAR_POPUP_PLANES, EVENTO_MOSTRAR_POPUP_TAREAS, EVENTO_MOSTRAR_HIJOS, PLANES_OK, PLANES_ERROR, TAREAS_BY_PLAN_ID_OK, TAREAS_BY_PLAN_ID_ERROR, MEDIA_CHANGE, SCREEN)(LitElement) {
	...
	stateChanged(state, name) {
		...
		if (name == SACAR_FILTRO_AMPAROS || name == PLAN_ADD) {
			this.itemsSeleccionados = [];
			this.rama = -1;
			this.filtro = false;
			store.dispatch(getPlanesAll());
